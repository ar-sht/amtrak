...
from ridership_analysis import load_station_ridership_by_code
...
def build_station_demand_lookup(graph):
    """Estimate station-level rider demand for accessibility calculations.

    We use observed station ridership where available. For unmatched stations,
    we backfill with weighted degree scaled into ridership units so every node
    participates in the rider-impact metric.
    """
    observed_ridership, unmatched = load_station_ridership_by_code()
    weighted_degree = dict(graph.degree(weight="weight"))

    ratios = []
    for node, ridership in observed_ridership.items():
        strength = weighted_degree.get(node, 0.0)
        if node in graph and strength > 0 and ridership > 0:
            ratios.append(ridership / strength)
    ...
    for node in graph.nodes():
        ridership = observed_ridership.get(node)
        if ridership is not None and ridership > 0:
            demand[node] = float(ridership)
        else:
            inferred_nodes.append(node)
            demand[node] = max(float(weighted_degree.get(node, 0.0) * scale), 1.0)

    return demand, unmatched, inferred_nodes


def ridership_weighted_accessibility(graph, station_demand):
    """Return total connected OD demand under a gravity-style approximation.

    Potential demand between two stations i and j is assumed proportional to
    demand[i] * demand[j]. A pair contributes only if both stations remain in
    the same connected component, making this a rider-centered analogue of
    pairwise connectivity.
    """
    total_connected_demand = 0.0

    for component in nx.connected_components(graph):
        component_demands = np.array(
            [station_demand[node] for node in component], dtype=float
        )
        if len(component_demands) < 2:
            continue
        demand_sum = component_demands.sum()
        squared_sum = np.square(component_demands).sum()
        total_connected_demand += 0.5 * (demand_sum**2 - squared_sum)

    return total_connected_demand
...
def compute_accessibility_vitality(graph, station_demand, top_n=10):
    """Rank stations by ridership-weighted accessibility loss when removed."""
    baseline = ridership_weighted_accessibility(graph, station_demand)
    vitality = {}

    for node in graph.nodes():
        g = graph.copy()
        g.remove_node(node)
        retained = ridership_weighted_accessibility(g, station_demand)
        vitality[node] = (baseline - retained) / baseline if baseline else 0.0

    ranked = sorted(vitality.items(), key=lambda x: x[1], reverse=True)
    return ranked[:top_n], vitality, baseline
...
def targeted_accessibility_removal(graph, ranked_nodes, station_demand, max_removals):
    """Track retained rider accessibility under a specified removal order."""
    g = graph.copy()
    retained = [ridership_weighted_accessibility(g, station_demand)]

    for node in ranked_nodes[:max_removals]:
        g.remove_node(node)
        retained.append(ridership_weighted_accessibility(g, station_demand))
...
def random_accessibility_removal(graph, station_demand, max_removals, trials=100):
    """Average retained rider accessibility under random removals."""
    all_nodes = list(graph.nodes)
    cumulative = [0.0] * (max_removals + 1)
    ...
    for _ in range(trials):
        g = graph.copy()
        sample = random.sample(all_nodes, min(max_removals, len(all_nodes)))
        cumulative[0] += ridership_weighted_accessibility(g, station_demand)

        for i, node in enumerate(sample):
            g.remove_node(node)
            cumulative[i + 1] += ridership_weighted_accessibility(g, station_demand)

    return [s / trials for s in cumulative]


def plot_robustness(
    targeted_sizes, random_sizes, max_removals, centrality, output_path=None
):
    ...
    ax.set_title(
        "Amtrak Network Robustness: Targeted vs Random Station Removal",
        fontsize=15,
        pad=12,
    )
...
def plot_accessibility_robustness(
    betweenness_retained,
    rider_retained,
    random_retained,
    betweenness_ranked_nodes,
    rider_ranked_nodes,
    output_path=None,
):
    """Compare rider accessibility under different attack strategies."""
    max_removals = (
        min(len(betweenness_retained), len(rider_retained), len(random_retained)) - 1
    )
    x = list(range(max_removals + 1))
    initial = rider_retained[0]

    fig, ax = plt.subplots(figsize=(12, 7))

    ax.plot(
        x,
        [100 * val / initial for val in random_retained],
        "s--",
        color="#2980b9",
        markersize=7,
        linewidth=2,
    ...
    ax.plot(
        x,
        [100 * val / initial for val in betweenness_retained],
        "o-",
        color="#7f8c8d",
        markersize=7,
        linewidth=2.2,
    ...
    ax.plot(
        x,
        [100 * val / initial for val in rider_retained],
        "o-",
        color="#c0392b",
        markersize=8,
        linewidth=2.5,
        label="Targeted removal (by rider accessibility loss)",
    )

    for i, node in enumerate(rider_ranked_nodes[:max_removals]):
        pct = 100 * rider_retained[i + 1] / initial
        ax.annotate(
            f"{node} ({pct:.0f}%)",
            xy=(i + 1, pct),
            fontsize=7.5,
            ...
            color="#8b0000",
            textcoords="offset points",
            xytext=(0, 16 if i % 2 == 0 else -18),
        )

    ax.set_xlabel("Number of stations removed", fontsize=13)
    ax.set_ylabel("Retained rider accessibility (%)", fontsize=13)
    ax.set_title("Rider Accessibility Under Station Removal", fontsize=15, pad=12)
    ax.legend(fontsize=11, loc="lower left")
    ax.set_xticks(x)
    ax.grid(True, alpha=0.25)

    footer = (
        f"Betweenness top 3: {', '.join(betweenness_ranked_nodes[:3])} | "
        f"Rider-loss top 3: {', '.join(rider_ranked_nodes[:3])}"
    )
    ax.text(
        0.99,
        0.02,
...
def plot_degree_distribution(graph, output_path=None):
    ...
    ax.text(
        0.97,
        0.95,
        f"{pct_pass:.0f}% of stations have\ndegree \u2264 2 (pass-through)",
        transform=ax.transAxes,
        ha="right",
        va="top",
        fontsize=10,
...
def plot_geographic_hubs(graph, centrality, top_n=10, output_path=None):
    ...
    edge_weights = [
        graph[u][v].get("weight", 1) for u, v in graph.edges() if u in pos and v in pos
    ]
    ...
    if hub_nodes:
        ...
        hub_colors = [
            plt.get_cmap("YlOrRd")(0.35 + 0.6 * (c / max_c)) for c in c_values
        ]
...
def plot_centrality_vs_degree(graph, centrality, top_n=10, output_path=None):
    ...
    ax.axhspan(
        cent_75,
        ax.get_ylim()[1] if ax.get_ylim()[1] > cent_75 else 0.6,
        xmax=0.35,
        alpha=0.06,
        color="#e74c3c",
    )
    ...
    ax.scatter(
        jittered_degrees[non_hub_mask],
        cent_values[non_hub_mask],
        s=20,
        alpha=0.4,
        color="#7f8c8d",
        zorder=2,
    ...
    for i, hub in enumerate(ranked[:top_n]):
        ...
        cy = cent_values[idx]
        ax.scatter(
            jx, cy, s=100, color="#e74c3c", edgecolors="black", linewidths=0.8, zorder=4
        )
...
def plot_edge_weight_distribution(graph, output_path=None):
    ...
    for i, (u, v, w) in enumerate(top_edges):
        ax.annotate(
            ...
            xytext=(0, 5 + i * 14),
            arrowprops=(
                dict(arrowstyle="->", color="#8b4513", lw=0.8) if i == 0 else None
            ),
        )
...
def print_top_rider_disruptors(top_stations):
    """Print a ranked table of stations by rider accessibility loss."""
    print(f"\n{'Rank':<6}{'Station':<12}{'Accessibility Loss':<20}")
    print("-" * 38)
    for i, (station, score) in enumerate(top_stations, 1):
        print(f"{i:<6}{station:<12}{score * 100:<19.2f}%")
...
if __name__ == "__main__":
    ...
    station_demand, unmatched_ridership, inferred_demand_nodes = (
        build_station_demand_lookup(G)
    )
    print(
        f"Loaded observed ridership for {len(station_demand) - len(inferred_demand_nodes)} stations; "
        f"used weighted-degree backfill for {len(inferred_demand_nodes)}."
    )

    top_hubs, centrality = compute_hub_centrality(G, top_n=MAX_REMOVALS)
    print_top_hubs(top_hubs)

    top_rider_disruptors, accessibility_vitality, baseline_accessibility = (
        compute_accessibility_vitality(G, station_demand, top_n=MAX_REMOVALS)
    )
    print_top_rider_disruptors(top_rider_disruptors)

    print(f"\nRunning targeted removal (top {MAX_REMOVALS} by betweenness)...")
    targeted_sizes = targeted_removal(G, centrality, MAX_REMOVALS)
    betweenness_ranked_nodes = [station for station, _ in top_hubs]
    rider_ranked_nodes = [station for station, _ in top_rider_disruptors]
    betweenness_accessibility = targeted_accessibility_removal(
        G, betweenness_ranked_nodes, station_demand, MAX_REMOVALS
    )
    rider_accessibility = targeted_accessibility_removal(
        G, rider_ranked_nodes, station_demand, MAX_REMOVALS
    )

    print("Running random removal (100 trials)...")
    random_sizes = random_removal(G, MAX_REMOVALS, trials=100)
    random_accessibility = random_accessibility_removal(
        G, station_demand, MAX_REMOVALS, trials=100
    )
    ...
    print(
        f"After removing {MAX_REMOVALS} random stations:   "
        f"largest component = {random_sizes[-1]:.0f} "
        f"({random_sizes[-1] / random_sizes[0] * 100:.1f}% of original)"
    )
    print(
        f"After removing {MAX_REMOVALS} betweenness-ranked stations: "
        f"retained rider accessibility = "
        f"{betweenness_accessibility[-1] / baseline_accessibility * 100:.1f}%"
    )
    print(
        f"After removing {MAX_REMOVALS} rider-loss-ranked stations: "
        f"retained rider accessibility = "
        f"{rider_accessibility[-1] / baseline_accessibility * 100:.1f}%"
    )
    print(
        f"After removing {MAX_REMOVALS} random stations: "
        f"retained rider accessibility = "
        f"{random_accessibility[-1] / baseline_accessibility * 100:.1f}%"
    )

    if not unmatched_ridership.empty:
        print(
            f"Ridership lookup left {len(unmatched_ridership)} spreadsheet rows unmatched; "
            "missing graph nodes were backfilled from weighted degree."
        )
    ...
    plot_robustness(
        ...
        MAX_REMOVALS,
        centrality,
        output_path=f"{out}/hub-robustness.png",
    )
    plot_degree_distribution(G, output_path=f"{out}/degree-distribution.png")
    plot_centrality_distribution(
        centrality, top_n=MAX_REMOVALS, output_path=f"{out}/centrality-distribution.png"
    )
    plot_geographic_hubs(
        G, centrality, top_n=MAX_REMOVALS, output_path=f"{out}/geographic-hubs.png"
    )
    plot_centrality_vs_degree(
        G, centrality, top_n=MAX_REMOVALS, output_path=f"{out}/centrality-vs-degree.png"
    )
    plot_edge_weight_distribution(G, output_path=f"{out}/edge-weight-distribution.png")
    plot_accessibility_robustness(
        betweenness_accessibility,
        rider_accessibility,
        random_accessibility,
        betweenness_ranked_nodes,
        rider_ranked_nodes,
        output_path=f"{out}/rider-accessibility-robustness.png",
    )
